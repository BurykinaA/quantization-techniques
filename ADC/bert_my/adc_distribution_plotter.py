...
class ADCDistributionPlotter:
    ...
    def __init__(self, save_dir: str = "./adc_distributions"):
        ...
        # Storage for batch data - expanded to include all quantization stages
        self.batch_data = {}
        self.batch_count = 0
    ...
    def log_batch_distributions(self, layer_name: str, 
        ...
        # Debug print to check actual values (reduced output)
        if step % 50 == 0:  # Only print every 50 steps
            print(f"ADC {layer_name}: diff_max={np.abs(after_np - before_np[:len(after_np)]).max():.6f}")
        
        self.batch_data[layer_name]['before_adc'].append(before_np)
        self.batch_data[layer_name]['after_adc'].append(after_np)
        self.batch_data[layer_name]['steps'].append(step)

    def log_full_quantization_pipeline(self, layer_name: str,
                                     x_raw: torch.Tensor,
                                     ...
                                     step: Optional[int] = None):
        """
        Log the complete quantization pipeline: raw inputs -> quantized inputs -> ADC
        
        Args:
            layer_name: Name of the layer
            x_raw: Raw input activations
            x_quantized: Quantized input activations
            w_raw: Raw weights
            w_quantized: Quantized weights
            before_adc: Tensor before ADC quantization (matmul result)
            after_adc: Tensor after ADC quantization
            step: Training step (optional)
        """
        if step is None:
        ...
        # Convert to numpy and flatten
        x_raw_np = x_raw.detach().cpu().numpy().flatten()
        x_quantized_np = x_quantized.detach().cpu().numpy().flatten()
        w_raw_np = w_raw.detach().cpu().numpy().flatten()
        w_quantized_np = w_quantized.detach().cpu().numpy().flatten()
        before_adc_np = before_adc.detach().cpu().numpy().flatten()
        after_adc_np = after_adc.detach().cpu().numpy().flatten()
        
        # Debug print to check actual values (reduced output)
        if step % 50 == 0:  # Only print every 50 steps
            x_diff = np.abs(x_quantized_np - x_raw_np[:len(x_quantized_np)]).max()
            w_diff = np.abs(w_quantized_np - w_raw_np[:len(w_quantized_np)]).max()
            adc_diff = np.abs(after_adc_np - before_adc_np[:len(after_adc_np)]).max()
            print(f"Full pipeline {layer_name}: x_diff={x_diff:.4f}, w_diff={w_diff:.4f}, adc_diff={adc_diff:.4f}")
        
        self.batch_data[layer_name]['x_raw'].append(x_raw_np)
        self.batch_data[layer_name]['x_quantized'].append(x_quantized_np)
        self.batch_data[layer_name]['w_raw'].append(w_raw_np)
        self.batch_data[layer_name]['w_quantized'].append(w_quantized_np)
        self.batch_data[layer_name]['before_adc'].append(before_adc_np)
        self.batch_data[layer_name]['after_adc'].append(after_adc_np)
        self.batch_data[layer_name]['steps'].append(step)
    ...
    def plot_full_quantization_pipeline(self, layer_name: str,
                                       x_raw: torch.Tensor,
                                       x_quantized: torch.Tensor, 
                                       w_raw: torch.Tensor,
                                       w_quantized: torch.Tensor,
                                       ...
                                       step: Optional[int] = None) -> str:
        """
        Plot the complete quantization pipeline distributions
        ...
        fig.suptitle(f'Full Quantization Pipeline - {layer_name} (Step {step})', fontsize=20)
        
        # Row 1: Input activations (x)
        # Plot 1: Raw vs Quantized activations
        ax1 = axes[0, 0]
        bins = 50
        ax1.hist(x_raw_np, bins=bins, alpha=0.6, label='Raw X', density=True, color='blue')
        ax1.hist(x_quantized_np, bins=bins, alpha=0.6, label='Quantized X', density=True, color='lightblue')
        ...
        # Plot 2: Activation quantization error
        ax2 = axes[0, 1]
        min_len = min(len(x_raw_np), len(x_quantized_np))
        x_diff = x_quantized_np[:min_len] - x_raw_np[:min_len]
        ax2.hist(x_diff, bins=bins, alpha=0.7, color='green', density=True)
        ax2.axvline(0, color='red', linestyle='--', alpha=0.7, label='No change')
        ax2.set_title('Activation Quantization Error')
        ax2.set_xlabel('Difference (Quantized - Raw)')
        ...
        ax3 = axes[0, 2]
        x_stats = {
            'Mean': [np.mean(x_raw_np), np.mean(x_quantized_np)],
            'Std': [np.std(x_raw_np), np.std(x_quantized_np)],
            'Min': [np.min(x_raw_np), np.min(x_quantized_np)],
            'Max': [np.max(x_raw_np), np.max(x_quantized_np)]
        }
        
        x_pos = np.arange(len(x_stats))
        width = 0.35
        for i, (stat, values) in enumerate(x_stats.items()):
            ax3.bar(i - width/2, values[0], width, label='Raw' if i == 0 else "", alpha=0.7, color='blue')
            ax3.bar(i + width/2, values[1], width, label='Quantized' if i == 0 else "", alpha=0.7, color='lightblue')
        
        ax3.set_title('Activation Statistics')
        ax3.set_ylabel('Value')
        ax3.set_xticks(x_pos)
        ax3.set_xticklabels(x_stats.keys())
        ax3.legend()
        ax3.grid(True, alpha=0.3)
        
        # Row 2: Weights (w)
        # Plot 4: Raw vs Quantized weights
        ax4 = axes[1, 0]
        ax4.hist(w_raw_np, bins=bins, alpha=0.6, label='Raw W', density=True, color='red')
        ax4.hist(w_quantized_np, bins=bins, alpha=0.6, label='Quantized W', density=True, color='lightcoral')
        ...
        # Plot 5: Weight quantization error
        ax5 = axes[1, 1]
        min_len = min(len(w_raw_np), len(w_quantized_np))
        w_diff = w_quantized_np[:min_len] - w_raw_np[:min_len]
        ax5.hist(w_diff, bins=bins, alpha=0.7, color='orange', density=True)
        ax5.axvline(0, color='red', linestyle='--', alpha=0.7, label='No change')
        ax5.set_title('Weight Quantization Error')
        ax5.set_xlabel('Difference (Quantized - Raw)')
        ax5.set_ylabel('Density')
        ax5.legend()
        ax5.grid(True, alpha=0.3)
        
        # Plot 6: Weight statistics
        ax6 = axes[1, 2]
        w_stats = {
            'Mean': [np.mean(w_raw_np), np.mean(w_quantized_np)],
            'Std': [np.std(w_raw_np), np.std(w_quantized_np)],
            'Min': [np.min(w_raw_np), np.min(w_quantized_np)],
            'Max': [np.max(w_raw_np), np.max(w_quantized_np)]
        }
        
        for i, (stat, values) in enumerate(w_stats.items()):
            ax6.bar(i - width/2, values[0], width, label='Raw' if i == 0 else "", alpha=0.7, color='red')
            ax6.bar(i + width/2, values[1], width, label='Quantized' if i == 0 else "", alpha=0.7, color='lightcoral')
        
        ax6.set_title('Weight Statistics')
        ax6.set_ylabel('Value')
        ax6.set_xticks(x_pos)
        ax6.set_xticklabels(w_stats.keys())
        ax6.legend()
        ax6.grid(True, alpha=0.3)
        
        # Row 3: ADC quantization
        # Plot 7: Before vs After ADC
        ax7 = axes[2, 0]
        ax7.hist(before_adc_np, bins=bins, alpha=0.6, label='Before ADC', density=True, color='purple')
        ax7.hist(after_adc_np, bins=bins, alpha=0.6, label='After ADC', density=True, color='plum')
        ax7.set_title('ADC Quantization')
        ax7.set_xlabel('Value')
        ax7.set_ylabel('Density')
        ax7.legend()
        ax7.grid(True, alpha=0.3)
        
        # Plot 8: ADC quantization error
        ax8 = axes[2, 1]
        min_len = min(len(before_adc_np), len(after_adc_np))
        adc_diff = after_adc_np[:min_len] - before_adc_np[:min_len]
        ax8.hist(adc_diff, bins=bins, alpha=0.7, color='magenta', density=True)
        ax8.axvline(0, color='red', linestyle='--', alpha=0.7, label='No change')
        ax8.set_title('ADC Quantization Error')
        ax8.set_xlabel('Difference (After - Before)')
        ax8.set_ylabel('Density')
        ax8.legend()
        ax8.grid(True, alpha=0.3)
        
        # Plot 9: Error metrics summary
        ax9 = axes[2, 2]
        
        # Calculate error metrics for each stage
        x_mse = np.mean(x_diff ** 2) if len(x_diff) > 0 else 0
        w_mse = np.mean(w_diff ** 2) if len(w_diff) > 0 else 0
        adc_mse = np.mean(adc_diff ** 2) if len(adc_diff) > 0 else 0
        
        x_mae = np.mean(np.abs(x_diff)) if len(x_diff) > 0 else 0
        w_mae = np.mean(np.abs(w_diff)) if len(w_diff) > 0 else 0
        adc_mae = np.mean(np.abs(adc_diff)) if len(adc_diff) > 0 else 0
        
        stages = ['Activation\nQuant', 'Weight\nQuant', 'ADC\nQuant']
        mse_values = [x_mse, w_mse, adc_mse]
        mae_values = [x_mae, w_mae, adc_mae]
        
        x_pos = np.arange(len(stages))
        ax9_twin = ax9.twinx()
        
        bars1 = ax9.bar(x_pos - width/2, mse_values, width, label='MSE', alpha=0.7, color='blue')
        bars2 = ax9_twin.bar(x_pos + width/2, mae_values, width, label='MAE', alpha=0.7, color='red')
        
        ax9.set_title('Quantization Error Summary')
        ax9.set_ylabel('MSE', color='blue')
        ax9_twin.set_ylabel('MAE', color='red')
        ax9.set_xticks(x_pos)
        ax9.set_xticklabels(stages)
        
        # Add value labels
        for i, (mse, mae) in enumerate(zip(mse_values, mae_values)):
            ax9.text(i - width/2, mse + mse*0.01, f'{mse:.4f}', ha='center', va='bottom', fontsize=8)
            ax9_twin.text(i + width/2, mae + mae*0.01, f'{mae:.4f}', ha='center', va='bottom', fontsize=8)
        ...
        if save_plot:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            safe_layer_name = layer_name.replace('/', '_').replace('.', '_')
            filename = f"full_pipeline_{safe_layer_name}_step_{step}_{timestamp}.png"
            filepath = os.path.join(self.save_dir, filename)
            plt.savefig(filepath, dpi=150, bbox_inches='tight')
            plt.close()
            print(f"Full pipeline plot saved: {filepath}")
            return filepath
        else:
            plt.show()
            return ""
    ...
    def plot_evolution_over_batches(self, layer_name: str, max_batches: int = 10):
        ...
        for i in range(n_batches):
            ...
            if 'before_adc' in data:
                before = data['before_adc'][i]
                after = data['after_adc'][i]
            else:
        ...
        if n_batches > 1:
            # Handle both old and new data structure
            if 'before_adc' in data:
                first_before = data['before_adc'][0]
                first_after = data['after_adc'][0]
                last_before = data['before_adc'][n_batches-1]
                last_after = data['after_adc'][n_batches-1]
            else:
            ...
            # Handle both old and new data structure
            if 'before_adc' in data:
                before = data['before_adc'][0]
                after = data['after_adc'][0]
            else:
