...
def plot_weight_distributions():
    """Finds all checkpoints, loads them, and plots their weight distributions."""
    config = get_config()
    ...
    print(f"Reading checkpoints from: {results_dir}")
    print(f"Saving plots to: {plots_output_dir}")

    checkpoint_files = [f for f in os.listdir(results_dir) if f.startswith('checkpoint_') and f.endswith('.pth')]
    
    if not checkpoint_files:
        print("No checkpoint files found to process.")
        return

    print(f"Found {len(checkpoint_files)} checkpoint file(s) to process.")
    ...
    for fname in checkpoint_files:
        ...
        match = re.match(r"checkpoint_(.*?)_(\d{8}_\d{6})\.pth", fname)
        if not match:
