#!/bin/bash
# run_overnight_int4_v12_bincenter.sh — INT4 v12: bin-center loss + multiplicative noise
#
# Same setup as v11 (M=256, multiplicative Gaussian noise before ADC), but adds the
# FlatQuant bin-center penalty during transform calibration:
#     L = L_FP + lambda * E[ cos^2(pi * y_int / delta) ]      (paper eq. 47)
# which pushes pre-ADC accumulated values toward bin centres (max margin 0.5*delta),
# making the post-ADC code less likely to flip under small perturbations / noise.
#
#   --fq_lambda_center 0.1
#   --fq_penalty_projections (all 7 projections)
#
...
#
# Usage: bash ADC/llama/run_overnight_int4_v12_bincenter.sh
# Results: ADC/llama/results/overnight_int4_v12_bincenter_YYYYMMDD.json
...
DATE_TAG="$(date +%Y%m%d)"
RESULTS_JSON="$RESULTS_DIR/overnight_int4_v12_bincenter_${DATE_TAG}.json"
LOG_DIR="$RESULTS_DIR/logs_v12_bincenter_${DATE_TAG}"
...
echo "============================================================"
echo "INT4 v12: bin-center loss (lambda=0.1, all 7 projs) + noise (M=256)"
echo "noise_std ∈ {0.1, 0.2, 0.5}"
echo "Started: $(date)"
...
OUTPUT_BASE="$SCRIPT_DIR/checkpoints/overnight_int4_v12_bincenter_${DATE_TAG}"
...
BASE_ARGS=(
    ...
    --fq_lambda_center 0.1
    --fq_penalty_projections q_proj k_proj v_proj o_proj gate_proj up_proj down_proj
    --fq_save_transforms
...
run_experiment() {
    ...
    python "$SCRIPT_DIR/runs/llama_smooth_quant_adc_ptq.py" \
        "${BASE_ARGS[@]}" \
        --output_dir "$out_dir" \
        --wandb_run_name "${name}_w4a4_int4_v12" \
        "$@" \
        > "$logfile" 2>&1
...
# ============================================================
# Sweep: multiplicative noise std ∈ {0.1, 0.2, 0.5} (M=256, bin-center on)
# ============================================================
