#!/bin/bash
# SmoothQuant + ADC PTQ Debug Script for LLaMA
#
# Runs extra diagnostics on top of the normal SmoothQuant + ADC PTQ pipeline:
#   1. FP16 baseline perplexity (before any changes)
#   2. SmoothQuant preprocessing (smooth activations/weights)
...
# SmoothQuant, quantization, tiling, or the ADC step.
...
WANDB_RUN_NAME="debug_sq_${MODEL_SHORT_NAME}_a${ALPHA}_bx${BX}_bw${BW}_ba${BA}_k${K}_${CALIBRATION_METHOD}"
SEED=42

echo "========================================"
echo "LLaMA SmoothQuant + ADC PTQ — DEBUG MODE"
echo "========================================"
echo "Model:        $MODEL_NAME"
echo "SmoothQuant:  Alpha=$ALPHA  Batches=$SMOOTH_QUANT_BATCHES"
...
echo "  [2] SmoothQuant preprocessing         (alpha=$ALPHA)"
echo "  [3] Quant+Tiling WITHOUT ADC          (--run_no_adc_eval)"
echo "  [4] Full SmoothQuant+ADC pipeline     (always)"
echo "========================================"
echo ""
...
echo "Running SmoothQuant + ADC PTQ with diagnostics..."
echo ""
eval $CMD
...
if [ $EXIT_CODE -eq 0 ]; then
    echo "Debug SmoothQuant + ADC PTQ Complete!"
    echo "========================================"
    echo ""
    echo "Check WandB for metrics comparison:"
    echo "  baseline/perplexity             — FP16 (no quantization, no SmoothQuant)"
    echo "  diagnostic/no_adc_perplexity    — SmoothQuant + Quant + Tiling (no ADC)"
    echo "  final perplexity                — Full SmoothQuant + ADC pipeline"
else
    echo "Debug SmoothQuant + ADC PTQ Failed (exit code $EXIT_CODE)"
    echo "========================================"
fi
echo "========================================"
